<?php $__env->startSection('title', 'Edit Peminjaman Tahunan'); ?>
        ...
        <div class="container-fluid">
            <div class="card mx-auto shadow" style="max-width: 800px;">
                <div class="card-header bg-primary text-white">
                    <h3 class="card-title"><i class="fas fa-edit"></i> Form Edit Peminjaman Tahunan</h3>
                </div>
                <form method="POST" action="<?php echo e(route('peminjamantahunan.update', $peminjaman->id)); ?>">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            <div class="col-md-12">
                                ...
                                <select name="siswas_id" id="siswas_id" class="form-control select2" <?php echo e(empty($siswaData) ? 'disabled' : ''); ?> required>
                                    ...
                                    <?php $__currentLoopData = $siswaData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $siswa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($siswa['id']); ?>" 
                                            ...
                                            <?php echo e($peminjaman->siswas_id == $siswa['id'] ? 'selected' : ''); ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" name="tanggal_pinjam" id="tanggal_pinjam"
                                    value="<?php echo e(\Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->format('Y-m-d')); ?>">
                            </div>

                            <div class="col-md-6">
                                <label for="tanggal_kembali" class="form-label">Tanggal Kembali</label>
                                <input type="date" class="form-control" name="tanggal_kembali" id="tanggal_kembali"
                                    value="<?php echo e(\Carbon\Carbon::parse($peminjaman->tanggal_kembali)->format('Y-m-d')); ?>">
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div id="kode-buku-container">
                                    <?php
                                        // id semua kode buku yang tersedia
                                        $availableKodeIds = $kode_bukus->pluck('id')->toArray();
                                    ?>
                                        ...
                                        <div class="input-group mb-2 kode-buku-item">
                                            <select name="kode_buku[]" class="form-control select2-single">
                                                <?php $__currentLoopData = $kode_bukus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <option value="<?php echo e($kode->id); ?>"
                                                        <?php echo e($detail->kode_bukus_id == $kode->id ? 'selected' : ''); ?>

                                                        <?php echo e($kode->status == 'dipinjam' && $kode->id != $detail->kode_bukus_id ? 'disabled' : ''); ?>>
                                                        <?php echo e($kode->kode_buku); ?> - <?php echo e($kode->buku->judul ?? '-'); ?>

                                                        <?php echo e($kode->status == 'dipinjam' && $kode->id != $detail->kode_bukus_id ? '(Masih Dipinjam)' : ''); ?>

                                                    </option>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                                                
                                                <?php if(!in_array($detail->kode_bukus_id, $availableKodeIds)): ?>
                                                    <option value="<?php echo e($detail->kodeBuku->id); ?>" selected>
                                                        <?php echo e($detail->kodeBuku->kode_buku); ?> -
                                                        <?php echo e($detail->kodeBuku->buku->judul ?? '-'); ?> (Dipinjam)
                                                    </option>
                                                <?php endif; ?>
                                            </select>
                                            ...
                                            <div class="input-group-append">
                                                <button type="button" class="btn btn-danger btn-sm"
                                                    onclick="this.closest('.kode-buku-item').remove()">
                ...
                <?php $__env->startPush('scripts'); ?>
                    ...
                    <script>
                        $(document).ready(function() {
                            ...
                            // Trigger filter on page load if student is already selected
                            const initialSiswaId = $('#siswas_id').val();
                            if (initialSiswaId) {
                                loadBukuByKelas(initialSiswaId, true); // true = preserve existing selections
                            }
                            ...
                            $('#siswas_id').on('change', function() {
                                ...
                                loadBukuByKelas(siswaId, false); // false = clear selections
                            });

                            function loadBukuByKelas(siswaId, preserveSelections) {
                                // Show loading indicator
                                $('.select2-single').prop('disabled', true);
                                
                                // Fetch filtered books via AJAX
                                $.ajax({
                                    url: '<?php echo e(route('peminjamantahunan.getBukuByKelas')); ?>',
                                    method: 'GET',
                                    data: { siswa_id: siswaId },
                                    success: function(data) {
                                        // Update all book dropdowns
                                        $('.select2-single').each(function() {
                                            const currentValue = preserveSelections ? $(this).val() : '';
                                            $(this).empty().append('<option value="">-- Pilih Kode Buku --</option>');
                                            
                                            // Add the current selected value first if preserving
                                            if (preserveSelections && currentValue) {
                                                const currentText = $(this).find('option:selected').text();
                                                $(this).append(new Option(currentText, currentValue, true, true));
                                            }
                                            
                                            data.forEach(function(item) {
                                                // Skip if already added as current value
                                                if (!preserveSelections || item.id != currentValue) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sistem-library\resources\views/peminjamantahunan/edit.blade.php ENDPATH**/ ?>