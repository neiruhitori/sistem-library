...
class PengembalianTahunanController extends Controller
{
    ...
    {
        $details = PeminjamanTahunanDetail::with(['peminjaman.siswa', 'kodeBuku.buku'])
            ->where('status', 'dipinjam')
            ->orderByDesc('created_at')
            ->get();

        return view('pengembaliantahunan.index', compact('details'));
    }
     ...
     */
    public function update(Request $request, PeminjamanTahunanDetail $detail)
    {
        DB::beginTransaction();
        try {
            ...
            foreach ($dendaList as $item) {
                CatatanDenda::create([
                    'siswas_id' => $siswaId,
                    'tipe_peminjaman' => 'tahunan',
                    'peminjaman_tahunans_id' => $detail->peminjaman->id,
            ...
            DB::commit();
            return redirect()->route('pengembaliantahunan.index')
