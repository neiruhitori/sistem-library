...
@section('title', 'Edit Peminjaman Tahunan')
    ...
    <div class="content">
        <div class="container-fluid">
            <div class="card mx-auto shadow" style="max-width: 800px;">
                ...
                <form method="POST" action="{{ route('peminjamantahunan.update', $peminjaman->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        @if (!$periodeAktif)
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" name="tanggal_kembali" id="tanggal_kembali"
                                    value="{{ \Carbon\Carbon::parse($peminjaman->tanggal_kembali)->format('Y-m-d') }}">
                            </div>
                                            ...
                                            <select name="kode_buku[]" class="form-control select2-single">
                                                @foreach ($kode_bukus as $kode)
                                                    <option value="{{ $kode->id }}"
                                                        {{ $detail->kode_bukus_id == $kode->id ? 'selected' : '' }}
                                                        {{ $kode->status == 'dipinjam' && $kode->id != $detail->kode_bukus_id ? 'disabled' : '' }}>
                                                        {{ $kode->kode_buku }} - {{ $kode->buku->judul ?? '-' }}
                                                        {{ $kode->status == 'dipinjam' && $kode->id != $detail->kode_bukus_id ? '(Masih Dipinjam)' : '' }}
                                                    </option>
                                                @endforeach

                                                {{-- Tambahkan jika kode buku yg dipilih tidak ada di list --}}
                                                @if (!in_array($detail->kode_bukus_id, $availableKodeIds))
                                                    <option value="{{ $detail->kodeBuku->id }}" selected>
                                                        {{ $detail->kodeBuku->kode_buku }} -
                                                        {{ $detail->kodeBuku->buku->judul ?? '-' }} (Dipinjam)
                                                    </option>
                                                @endif
                                            </select>
